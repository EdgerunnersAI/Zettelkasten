...
## iter-12 final code-deploy validation (2026-05-07)

Deploy chain (3 hotfix hops):
- Bandit migration idempotent CREATE prepend (3a495af)
- Aliases IMMUTABLE wrapper + GENERATED column + trgm tier in resolver (45328a8)
- L2 CI gate: `.github/workflows/migration-ci.yml` applies all migrations to fresh `postgres:16-alpine` per PR — catches future ordering / IMMUTABLE-in-index / missing-extension bugs before deploy. (054761f)

Final deploy: master HEAD 054761f, GHA run 25514400396 (deploy-droplet.yml: success), CI run 25514400405 (success), /api/health 200.
Live env capture: docs/rag_eval/common/knowledge-management/iter-12/_audit/live_env_capture.txt

Active color: green. 7/7 required knobs confirmed:
- RAG_ANCHOR_BOOST_ENABLED=true
...
Burst-12 probe result: total=2124, 502_rate=0.0, 503_rate=0.0, p50=141ms, p95=953ms, p99=6422ms, post-burst lag p95=0.964ms — verdict: **PASS**

Note on migration-ci.yml: the workflow triggered on this push but the `paths` filter (`supabase/**/*.sql`) did not match (only `.github/workflows/migration-ci.yml` was changed), so no migration-ci run fired on this commit. It will fire on all future PRs/pushes that touch any `supabase/**/*.sql` file.

Operator next step (PowerShell — when ready to run iter-12 eval):
```powershell
# PowerShell
...
Iter-12 task chain complete: T34, T23, T24, T1, T2, T3 (Phase 1 burst-12 PASS), T5 (Phase 2 anchor flip), T6, T7, T8, T9, T11+T12+T13, T14, T15, T18, T25, T35, T28, T29, T30, T31, T19, T26, T32, T33, T36 — 27 of 29 (T20 = pytest sanity ✓; T22 = scores.md generated by operator post-eval).
